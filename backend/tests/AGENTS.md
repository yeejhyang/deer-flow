...
`test_executor_starvation.py` covers only deterministic executor starvation semantics from RFC #4560:
...
Use `threading.Event` or equivalent explicit synchronization rather than sleep-based timing thresholds for worker lifecycle assertions. Every test must release blocked workers and restore any monkeypatched executor so teardown cannot leak threads or state into later tests.
